...
board_print( null , null ); // 1. js 시작될때 함수실행

// 1. 게시물 출력 함수
	// 인수 : 1. 페이지 처음 js열렸을때[ 조건x : keyword=null , key=null ]
	//		 2. 검색되었을때[ 조건o : keyword=입력받은 값 , key=입력받은 값 ]
	//		 3. 페이지전환되었을때
function board_print( keyword , key ){
	// 1. JAVA[백엔드]로 부터 데이터 요청 //~추후
	//if( keyword == null && key == null ){ // 1. 검색이 없는 경우
		//alert('검색이 없는 게시물 출력')
		// JAVA에게 검색[조건]이 없는 게시물들 요청
	//}else{ // 2. 검색이 있는경우
		//alert('검색이 있는 게시물 출력')
		// JAVA에게 keyword , key 보내서 검색[조건]이 없는 게시물들 요청
	//}
	...
	boardlist.forEach( (board) => {
		html += `<tr>
					<td>${ board.no }</td><td><a href="view.html">${ board.title }</a></td>
