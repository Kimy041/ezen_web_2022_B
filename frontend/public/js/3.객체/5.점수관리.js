...
addbtn.addEventListener( 'click' , () => {
	...
	// 2. 유효성검사 [ 데이터 체크 ]
	let check = true;	// 유효성검사 상태 저장하는 변수 [ 아래 4개 하나라도 충족하면 저장 실패 ]
		// 1. 이름 중복체크
	...
	if( (info.kor < 0 || info.kor > 100) ||
		...
		alert('입력할 수 없는 점수 범위 입니다.[ 0~100 사이만 가능 ]'); check = false;
	}// if end
	...
	if( info.name.length < 3 || info.name.length > 5 ){
		...
		// 4. 숫자 아닐경우 [ isNaN() : 숫자  체크 [ 문자이면 true or 숫자이면 false ] ]
...
/*for( let i = 0 ; i <= info.name ; i++ ){
		hmtl +=`<tr>
				...
				<th>${info.eng}</th>
				<th>${info.mat}</th>
				<th>${info.mat+info.eng+info.kor}</th>
				<th>${(info.mat+info.eng+info.kor)/3}</th>
				<th>순위</th>
