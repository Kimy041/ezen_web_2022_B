...
// 3. 결과 함수 [ 사람/컴퓨터 알두기성공했을때 1. 승리 2. 패배 3.무승부[빈자리없으면] ]
function 결과(){ 
	// 1. 가로 승리자 판단
		// * 배열의 초기값이 모두 null 이기 때문에 null 제외
	for( let i = 0 ; i <=6 ; i+=3 ){ // i는 0부터 6까지 3씩중가 반복처리 -> 3회반복 : 0 3 6
		if( 버튼목록현황[i] != null && 버튼목록현황[i] == 버튼목록현황[i+1] && 버튼목록현황[i+1] == 버튼목록현황[i+2]){ 
			결과출력( 버튼목록현황[i] )	// 버튼목록현황[i] : 승리자 알모양
			return true; } // * 승리자 존재 하는 신호;
	}// for e
	// 2. 세로 승리자 판단
	for( let i = 0 ; i <=2 ; i++ ){ // i는 0부터 2까지 1씩중가 반복처리 -> 3회반복 : 0 1 2
...
/*
	...
	- 무승부 : 알이 모두 존재 했을때 = null 0개이면 -> 빈자리가 없다.
*/
// 4. 게임 승리자 출력
function 결과출력( 알모양 ){
	...
	if( 알모양 == '-'){ // 만약에 알 모양이 '-' 무승부
		게임판.innerHTML = '무승부'
	}else{ // 만약에 알 모양이 '-' 무승부 아니면 승리자 알 'O' or 'X'
