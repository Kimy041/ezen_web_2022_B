...
public class Boardinfo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( type == 1 ) { // 1.전체 출력
			// ------------- 카테고리 별 출력 --------------- //
			// 1. 카테고리 매개변수 요청 [ cno ]	2. gettotalsize/getBordList 조건 전달
			int cno = Integer.parseInt( request.getParameter("cno") );
			
			// -------------- 검색 처리 -----------------//
			// 1. 검색에 필요한 매개변수 요청 [ key , keyword ]	2. gettotalsize/getBordList 조건 전달
			String key = request.getParameter("key");
			String keyword = request.getParameter("keyword");
			
			// -------------- page 처리 ----------------//
			// 1. 현재페이지[ 요청 ] , 2. 페이지당 표시할게시물수 , 3. 현재페이지[ 게시물시작 , 게시물끝 ]
			int page = Integer.parseInt( request.getParameter("page") );
			// int listsize = 3;
			int listsize = Integer.parseInt( request.getParameter("listsize") ); // 회면에 표시할 게시물수 요청
...
*/
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/* 첨부파일의 수정 경우의 수
			1. 기존 첨부파일이 없었다.	--> 새로운첨부파일 없다.	[x]
								--> 새로운첨부파일 있다.	[ 업로드 , 새로운파일로 업데이트 처리 ] - dao
			2. 기존 첨부파일이 있었다.	--> 새로운첨부파일 없다.	[ 기존파일명으로 업데이트 처리 (그대로 사용) ]
								--> 새로운첨부파일 있다. 	[ 업로드 , 새로운파일로 업데이트 처리 , 기존파일 삭제 ] - dao , 삭제
		*/
		// 1. 수정전 기존 첨부파일명 가져오기
		String oldfile = BoardDao.getInstance().getBoard(bno).getBfile();
		if( bfile == null) { // 새로운 첨부파일이 없디.
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// [공통] 파일삭제
		if( result ) { // 민약에 db가 레코드를 삭제를 성공하면
...
//삭제/수정시 : 첨부파일 있을경우 같이 삭제
			// 1. 경로 찾아서
			// 2. 파일 객체회[?? 다양한 파일 관련 메소드 제곤 .length() , .delete() , .exists() 등 ]