...
public class FiledownLoad extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 3. HTTP 다운로드 형식[프론트엔드] = 각 브라우더(크롬, 엣지 등) 제공
		...
		BufferedOutputStream fout = new BufferedOutputStream( response.getOutputStream() );
					// 2. 파일입력스트림객체레서 읽어온 바이트들[바이트배열]
					// 파일출력스트림개게 write() : 해당 배열내 바이트를 출력하는 함수
		fout.write(bytes); // ---> response.getOutputStream()
				// 3. 스트림 닫기[ CG 대신 직접 스트림 닫기 ]
		fin.close(); 	// 파일입력스트림객체 스트림 닫기
		fout.flush(); 	// 피일입력스트림객체 스트림 메모리 초기화
		fout.close(); 	// 파일입력스트림객체 스트림 닫기
	}
