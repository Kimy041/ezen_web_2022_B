...
public class BoardDao extends Dao  {
	...
	public ArrayList<BoardDto> getBoardList(int startrow , int listsize , String key , String keyword , int cno ){
		...
		String sql = "";
		if( key.equals("") && keyword.equals("") ) { // 검색이 없다
			sql = "select b.* , m.mid from member  m natural join board b where b.cno = "+cno+" order by b.bno desc limit ? ,? ";
		}else { // 검색이 있다
			sql = "select b.* , m.mid from member  m natural join board b  where "+key+" like '%"+keyword+"%' and b.cno = "+cno+" order by b.bno desc limit ? , ?";
		}
