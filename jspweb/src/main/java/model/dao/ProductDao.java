...
public class ProductDao extends Dao {
	...
	public HashMap<String, Integer> getSum(){
		...
		String sql = "select sum( if( mpcomment = '포인트 충전' , mpamount , 0 ) ) as 충전된포인트총합계 , date_format( mdate , '%y%m%d' ) as 충전날짜 from mpoint group by  date_format( mdate , '%y%m%d' )  order by 충전날짜 desc limit 5";
		try {
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			while( rs.next() ) {
...
/*
	...
	ArrayList< 타입 > list = new ArrayList<>();
		...
		.add( 데이터 )
	// 2. 해당 키타입과 데이터타입의 해당하는 키와 데이터를 여러개 저장할수 있는 맵 객체 선언
