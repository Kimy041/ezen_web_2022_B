...
public class ProductDao extends Dao {
	...
	// 6. 제품에 등록 채팅 [ 제품번호 일치, 현재 보고 있는 회원[로그인된회원] 받거나 보낸 내용들]
	public ArrayList<ChatDto> getCahtList( int pno , int mno ){
		...
		try {
			...
			rs = ps.executeQuery();
			while( rs.next() ) {
				list.add( new ChatDto( rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5), rs.getInt(6)));
			}
		}catch (Exception e) { System.out.println(e); }
		return list;
	}
