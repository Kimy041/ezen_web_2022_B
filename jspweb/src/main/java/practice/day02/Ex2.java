...
public class Ex2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String jsonArray = objectMapper.writeValueAsString(list);
			// System.out.println( list ); // 자바객체
				// [ 주소 , 주소 ]
			// System.out.println( jsonArray ); // 자바 객체가 json형식으로 문자열 반환
				// [ { } , { } ]
		// 2. 요청으로 받은 결과물 JS에게 전달
		response.setCharacterEncoding("UTF-8");
		response.setContentType("application/json");	// 전달[전송]타입릉 json 명시
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. request.getParameter("매개변수명");	: 데이터 가져올때 무조건 String 타입 가져온다. [ 필요에 따라 형 변환 ]
