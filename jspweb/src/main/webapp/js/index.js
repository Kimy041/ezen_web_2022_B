...
function productprint( i ){
	...
	html += `
			...
			<div class="pstate"> <span class="pbadge"> ${ p.pstate == 1 ? '판매중' : p.pstae == 2 ? '거래중' : '판매완료' } </span> </div>
...
function getplike( pno ){
	...
	//$.ajax({ url : "/jspweb/product/like" , {"data" : data } , success : (r)=>{ console.log(r); } })
	
	// $.post("/jspweb/product/like, {"data" : data } , (r)=>{})
	// $.ajax({ url : "/jspweb/product/like" , method : "post" , data : { "pno" : pno } , success : (r)=>{ console.log(r); } })
