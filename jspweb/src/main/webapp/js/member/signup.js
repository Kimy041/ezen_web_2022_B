	...
	JS 정규표현식 : 문자 특징 규칙 , 패턴 , 집합 표현할때 사용되는 언어
		-- 문법
			...
			* : 앞 에 있는 패턴 0개 반복
			...
			--> 영문1개 + 숫자1개 필수로 갖는 5~20글 사이
			
			/^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])[A-Za-z0-9]{5,20}$/
			--> 영대문자1개 + 영소문자1개 + 숫자1개 필수로 갖는 5~20글 사이
...
function idcheck(){ // onkey : 키 누르고 떼었을때
	// 1. 입력할때마다 입력값 가져오기
	let mid = document.querySelector('.mid').value;
	// 2. 정규표현식 [ 영문(소문자)+숫자 길이 5~30 글자 ]
	let midj = /^[a-z0-9]{5,30}$/
	// 3. 정규표현식 검사
	if( midj.test(mid) ){ // 정규표현삭 패턴이 true 이면
...
// 3. 비밀번호 유효성검사
function pwdcheck(){ 
	let mpwd = document.querySelector('.mpw').value; // 1. 입력받은 값 가져오기
	// 2. 정규표현삭 : 영대소문자+숫자 조합 5~20 글자
...
function pwdconfirmcheck(){ 
	...
	let mpwdconfirm = document.querySelector('.mpwconfirm').value;
...
function emailcheck(){
	...
	if( memailj.test(memail) ){
		...
		document.querySelector('.authbtn').disabled = true;		// 인증 버튼 사용 불가능
	}
		
}
// 6.  이메일 인증 함수
function getauth(){
	console.log('getauth()함수')
	// ----------- 1. 메일 전송 테스트 할 경우 -----------------//
	...
	*/
	...
	document.querySelector('.authbox').innerHTML = html
	// 3. 타이머 함수 실행
	auth =1234;		// **인증 임시 코드 대입	[ 이메일에게 보낸 난수 대입 ]
	timer = 120;	// 인증 시간 대입
	settimer();		// 타이머 함수 실행
