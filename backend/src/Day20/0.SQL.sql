...
-- 1. 아이디 중복체크 [ 해당 아이디 검색 ]
	-- 검색후 레코드가 존재하면 아미 존재하는 아이디/ 레코드가 없으면 존재하지 않은 아이디
...
-- 3. 로그인
	-- 검색한 결괴의 레코드가 존재하면 로그인성공/ 결과 레코드가 존재하지않으면 로그인 실패
select * from member where mid = 'qwe' and mpw = '123'; -- 아이디와 비밀번호가 일치한 회원 레코드 찾기
...
select * from board order by bdate desc; -- 작성일 기준 최근 날짜로 정렬 [ DESC:현재->과거  ASC:과거->현재 ]
...
-- 6. 해당 카테고리 모든 게시물 출력 [ 1반 카테고리 게시물 들 ]
select * from board where cno_fk = 1;
	-- 
		select b.bno , b.btitle , b.bcontent , b.bdate , b.bview , m.mid , c.cname
        from board b , member m , category c
        where b.mno_fk = m.mno and b.cno_fk = c.cno and b.cno_fk = 1;
