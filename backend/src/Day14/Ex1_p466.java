...
public class Ex1_p466 {
	...
	public static void printLength(String data) {
		try { // * 예외 발생할것 같은 코드를 try { } 안에 넣기
			...
			// * 만약에 try { } 안에서 예외가 발생했을때 실행되는 catch { } 코드
				// - 만일 try{} 안에서 발생하지 않으면 catch { } 실행 안됨.
				// e : 예외객쳬명[아무거나] 예외정보가 남겨져 있는 객체
			System.out.println( e.getMessage() );
		}
		finally { // * 예외가 있던말전 실행되는 코드
	...
	public static void main(String[] args) {
		...
		printLength( null ); // NullPointerException : 데이터가 null 이라서 length() 
...
/*
	...
	문번
		try{
			...
			2. try{} 에서 예외가 발생했을때 실행되는 코드 [ 만일 예외발생했을때 이루 실행코드 ]
		}
		finally{
