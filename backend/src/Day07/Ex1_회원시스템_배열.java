	...
	public static void main(String[] args) { // main s
		...
		while(true) { // 무한루프 [ 종료조건 : 없음 ]
			...
			if( ch == 1 ) { // 1. 1입력했을때
				...
				for( int i = 0 ; i<memberList.length ; i++ ) {
					if( memberList[i] == null ) { // 만약에 i번째 인덱스가 null 이먄
						memberList[i] = name+","+phone; // 이름과 전화번로 1개의 문자열 합치기
						System.out.println(i+"회원이 등록 되었습니다. 정보 : "+ memberList[i] );
